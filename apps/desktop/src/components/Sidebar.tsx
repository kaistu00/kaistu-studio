...
export type ViewPath = "projects" | "text" | "image" | "audio" | "video" | "library" | "settings";
...
const SETTINGS_ITEM: NavItem = { id: "settings", label: "Configuración", icon: "settings" };
  ...
  return (
    <aside className={"sidebar" + (collapsed ? " collapsed" : "")}>
      ...
      <div className="sidebar-bottom">
        <button
          className={"nav-btn" + (active === SETTINGS_ITEM.id ? " active" : "")}
          onClick={() => onNavigate(SETTINGS_ITEM.id)}
          title={collapsed ? t(SETTINGS_ITEM.label) : undefined}
        >
          <span className="nav-icon material-symbols-outlined">{SETTINGS_ITEM.icon}</span>
          {!collapsed && <span className="nav-label">{t(SETTINGS_ITEM.label)}</span>}
        </button>
